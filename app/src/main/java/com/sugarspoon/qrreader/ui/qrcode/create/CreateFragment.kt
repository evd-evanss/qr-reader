...
import com.sugarspoon.qrreader.base.App
import com.sugarspoon.qrreader.base.BaseFragment
import com.sugarspoon.qrreader.data.model.VirtualCard
import com.sugarspoon.qrreader.ui.qrcode.card.VirtualCardFragment
...
class CreateFragment :
    ...
    private val presenter: CreateContract.Presenter by lazy {
    ...
    override fun setViews()  = view?.run {
        requireActivity().run {
            createCardGenerateLv.setView(LoadingView.LoadingIntent.CreatePixIntent())
            setToolbar(getString(R.string.create_card), false)
        }
    }
    ...
    override fun generateVirtualCard(virtualCard: VirtualCard) {
        view?.run {
            val bundle = Bundle()
            bundle.putSerializable("virtualCard", virtualCard)
            val intent = Intent(requireContext(), VirtualCardFragment::class.java)
